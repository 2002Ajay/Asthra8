...
model = load_model("FER_model.h5", custom_objects=dependencies)
...
while(True):
    ...
    # Resize the grayscale image to the required input shape (48, 48)
    resized_gray_img = cv2.resize(gray_img, (48, 48))
    # Expand the dimensions of the grayscale image to match the expected input shape
    input_img = np.expand_dims(resized_gray_img, axis=0)

    display = np.array(makeDisplayGrid(input_img)) / 255.
